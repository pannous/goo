...
var cases = []testCase{
	...
	},
	{
		...
		checks: []checks{hasAttr("k", "replaced")},
	},
	{
		name:        "resolve-groups",
...
func TestHandler(h slog.Handler, results func() []map[string]any) error {
	...
	for i, got := range res {
		c := cases[i]
		for _, checks := range c.checks {
			if problem := checks(got); problem != "" {
...
type checks func(map[string]any) string

func hasKey(key string) checks {
...
func missingKey(key string) checks {
	return func(m map[string]any) string {
		if _, ok := m[key]; ok {
...
func hasAttr(key string, wantVal any) checks {
	return func(m map[string]any) string {
		if s := hasKey(key)(m); s != "" {
			return s
		}
...
func inGroup(name string, c checks) checks {
	return func(m map[string]any) string {
		v, ok := m[name]
		if !ok {
