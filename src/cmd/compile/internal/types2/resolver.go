...
func (checks *Checker) collectObjects() {
	...
	for fileNo, file := range checks.files {
		...
		if strings.HasSuffix(file.PkgName.Pos().RelFilename(), ".goo") {
			...
			// NOTE: strings and slices auto-imports are now handled at the package loading level
			// in cmd/go/internal/load/pkg.go for proper importcfg integration
		}
...
func (checks *Checker) errorUnusedPkg(obj *PkgName) {
	...
	if obj.name == "" || obj.name == "." || obj.name == elem {
		checks.warningf(obj, UnusedImport, "%q imported and not used (warning only)\n", path)
	} else {
		checks.warningf(obj, UnusedImport, "%q imported as %s and not used (warning only)\n", path, obj.name)
	}
}
...
func injectSlicesImportIfNeeded(file *syntax.File, fileDir string, checks *Checker, fileScope *Scope) {
	...
	if needsSlicesImport {
		...
		// CRITICAL: Add the import declaration to file.DeclList so later compilation stages can find it
...
func injectStringsImportIfNeeded(file *syntax.File, fileDir string, checks *Checker, fileScope *Scope) {
	...
	for _, decl := range file.DeclList {
		if importDecl, ok := decl.(*syntax.ImportDecl); ok {
			if importDecl.Path != nil && strings.Contains(importDecl.Path.Value, "strings") {
				println("DEBUG: strings already imported, skipping")
	...
	// If strings not imported, scan for string method usage
	if !hasStringsImport {
		syntax.Inspect(file, func(n syntax.Node) bool {
			if call, ok := n.(*syntax.CallExpr); ok {
				if selector, ok := call.Fun.(*syntax.SelectorExpr); ok {
	...
	// Inject import "strings" by processing it exactly like a regular import
	if needsStringsImport {
		println("DEBUG: Injecting strings import")
		// Create synthetic import declaration
		importPath := &syntax.BasicLit{
			Value: `"strings"`,
			Kind:  syntax.StringLit,
		...
		stringsImport.SetPos(importPath.Pos())
		...
		println("DEBUG: Attempting to import strings from path:", path, "fileDir:", fileDir)
		imp := checks.importPackage(importPath.Pos(), path, fileDir)
		if imp == nil {
			println("DEBUG: Import failed - strings package not found")
			// Import failed - don't inject the import declaration
			return
		}
		println("DEBUG: Strings import successful")

		// Create package name - use the imported package name
		pkgName := NewPkgName(stringsImport.Pos(), checks.pkg, imp.name, imp)
