...
func (t *InOperatorTransform) convertInOperation(op *syntax.Operation, visitor *inVisitor, file *syntax.File) syntax.Expr {
	...
	case "string":
		return t.createStringContainsCall(op, visitor, op.Pos())
    case "slice":
        return op
	case "map":
...
func (t *InOperatorTransform) isMapInOperation(expr syntax.Expr, ctx *TransformContext) (*syntax.Operation, bool) {
	...
	if t.inferContainerType(op.Y, ctx) == "map" {
		return op, true
	}
	return nil, false
}
...
func (t *InOperatorTransform) rewriteCheckWithStringIn(cs *syntax.CheckStmt, ctx *TransformContext) (syntax.Stmt, bool) {
	...
	if t.inferContainerType(op.Y, ctx) != "string" {
		return nil, false
	}
	...
	runePos := pos
	if !runePos.IsKnown() {
		runePos = generatedNodePos(nil)
	}
	if t.isRuneLiteral(op.X) {
		nRhs = t.convertRuneToString(op.X, runePos, nil)
	}
...
func addImportIfMissing(file *syntax.File, path string) bool {
	quoted := path
	...
	importDecl := &syntax.ImportDecl{
		Path: &syntax.BasicLit{Value: quoted, Kind: syntax.StringLit},
	}
	...
	newDecls := make([]syntax.Decl, 0, len(file.DeclList)+1)
	newDecls = append(newDecls, file.DeclList[:insertPos]...)
	newDecls = append(newDecls, importDecl)
	newDecls = append(newDecls, file.DeclList[insertPos:]...)
	file.DeclList = newDecls
	return true
}
...
func (t *InOperatorTransform) convertRuneToString(runeExpr syntax.Expr, pos syntax.Pos, file *syntax.File) syntax.Expr {
	if !pos.IsKnown() {
		pos = generatedNodePos(file)
	}
